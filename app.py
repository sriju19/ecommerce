...
db_init(app)#encrypting database
...
@app.route("/products")
def get_products():
    # Query all rows from the Product table
    products = Product.query.all()

    # Render a template to display the products
    return render_template("products.html", products=products)

@app.route("/delete_products")
def delete_products():
    try:
        # Remove all data from the Product table
        db.session.query(Product).delete()
        db.session.commit()
        return "All products deleted successfully."
    except Exception as e:
        return f"Error deleting products: {str(e)}"
